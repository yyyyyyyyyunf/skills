...
3. **Branch — is this a multi-session build?**
   - **Yes** → **`/to-spec`** (turn the thread into a spec), then **`/to-tickets`** to split it into tracer-bullet tickets, each declaring its **blocking edges** and agreed verification. How those edges are physically expressed is tracker-specific — see `docs/agents/issue-tracker.md`, written by `/setup-agent-workflow`. An agent-ready ticket whose blockers are done can be grabbed: kick off **`/implement`** per ticket, **`/clear`ing context between each one**. Each ticket is self-contained, so the last one's context is disposable.
   - **No** → **`/implement`** right here, in the same context window.

   Either way, **`/implement`** runs `acceptance-plan` first to bind the agreed contract and capture any required baseline, then `tdd` where appropriate, `code-review`, and `acceptance`. Acceptance returns `passed` or `held`; tracker finalization follows that verdict. A project may use test scripts, API assertions, agent browser observation, or another agreed proof method. For unattended execution, **`/setup-agent-workflow`** connects the selected runner's prompt and continuation rules to this workflow and checks environment and ticket readiness. You need no separate manual invocation of its acceptance stages.
...
- **Bugs and requests piling up** → **`/triage`**. It moves issues through triage roles and produces agent-ready issues, which **`/implement`** later picks up.
  ...
  Triage is for issues **you didn't create** — bug reports, incoming feature requests, anything that arrives raw. `/to-tickets` reviews acceptance and readiness during planning, so its output does not need another triage pass; tickets with unresolved prerequisites or decisions keep those conditions visible.
...
- `acceptance-plan` — binds the agreed outcomes and proof methods to the execution environment before implementation. It checks prerequisite providers and captures old-behaviour baselines only for checks that require them. New features can use expected results without a baseline. Reads the project's editable `docs/agents/acceptance.md`; `/implement` invokes it automatically.
- `acceptance` — executes the agreed proof and reports `passed` or `held`, with evidence judged by executable assertions, an independent agent, or explicitly required human approval. Required criteria and engineering gates must all pass; advisory findings and missing proof stay visible. Reach for it directly to verify a finished change.
...
- **`/setup-agent-workflow`** — configure a repo or migrate it to updated workflow conventions. Configures the issue tracker, triage labels, doc layout, and project acceptance workflows in `docs/agents/`. Optionally connects an existing or selected unattended runner; Sandcastle uses its official initializer and an adapter for ticket handoff, results, and stopping. Checks existing-ticket and execution-environment gaps without claiming that configuration alone makes them runnable. Backlog.md, GitHub, GitLab, local markdown, and custom tracker workflows are supported.
