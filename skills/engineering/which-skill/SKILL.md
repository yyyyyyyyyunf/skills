...
3. **Branch — is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue in four stages, all agent-fired: `acceptance-plan` first, fixing what counts as done before any code moves (and freezing the baseline when existing behaviour must survive), then `tdd` — one red-green slice at a time — then `code-review`, a two-axis review (Standards + Spec) of the diff, then `acceptance`, which runs the code and reports coverage before the commit. You get all four without asking; reach for one by name when you want it on its own — a concrete behaviour built test-first with no spec, a branch reviewed against a fixed point, a finished change you want proven.
...
- `acceptance-plan` — fixes what counts as done *before* any code moves, and freezes the baseline while it still exists. `/implement` fires it first, but reach for it directly the moment you know a migration or framework change is coming and haven't decided how yet: **the baseline window closes on the first edited line**, and after that equivalence is unprovable for the rest of the effort. Reads `docs/agents/acceptance.md`, written by `/setup-agent-workflow`.
- `acceptance` — runs the finished code and proves it with reproducible evidence, judged by a **blind** sub-agent that never sees the diff. Its value is the honesty of its numbers in both directions: coverage beside `blocked` and `gap` counts, so "37 changed behaviours have no proof at all" can't hide inside a green. Reach for it directly to prove a change that's already written.
