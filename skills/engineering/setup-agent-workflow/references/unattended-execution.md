...
Follow the actual launch command through its entry script, prompts, hooks, and imported package version. Read the installed API and template before adapting them. For Sandcastle, use [the native integration guide](sandcastle.md). Other runners can use their native result and lifecycle mechanisms to satisfy the same project contract.
...
- Its task prompt loads and follows the installed `implement` skill for the host-selected ticket, with the ticket/spec context and a fresh host-generated attempt ID.
...
After an attempt, the default policy verifies `completed` and finalization before merging the checked candidate, then confirms merge/cleanup and the durable record before starting another. `held`, `incomplete`, a missing/invalid result, or a preserved worktree stops the run for inspection. `no-work` ends normally; a wholly `blocked` queue ends with its blockers reported. Keep retries bounded within the implementation/acceptance workflow; do not automatically restart a held ticket in fresh worktrees. A project-specific continuation policy must be explicitly agreed and still stop on persistence, merge, cleanup, or result-validation failures.
...
Bind host checks to the original approved contract and the exact candidate/target revisions. Verify local tracker/report state and evidence before merge; confirm destination persistence and cleanup before proceeding. A remote tracker must confirm its required state. Held/incomplete, invalid results or changed revisions cannot deliver a candidate. An installed runner that cannot satisfy this order remains unverified or blocked; report the capability gap instead of adding a wrapper that only notices problems after merge.

Store required evidence outside disposable worktrees, or copy it before cleanup. Gitignored evidence inside a clean worktree is still deleted with that worktree. Declare the exported evidence paths and persistent artifact root; align report references and project acceptance paths with that mapping. For the native Sandcastle workflow, the agent writes relative evidence paths in its worktree and the runner exports them before checking/cleanup. Use a documented lifecycle point or a persistent mount/path, and prove retrieval after cleanup. When durable storage is unavailable, report the limitation and hold affected delivery rather than claiming the evidence is preserved.
...
Verify generated or modified native configuration with a fake task executor and temporary repositories/fixtures first. Exercise observable continuation, persistence, and cleanup, rather than matching instruction text. Run the project's relevant checks for any added executable code.
