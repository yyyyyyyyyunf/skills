...
- `backlog.config.yml` at the repo root, or a `backlog/` directory — sign that Backlog.md is already the tracker here. If either exists, run `backlog config` to read the real status vocabulary; Section A needs it. Also run `command -v backlog` to learn whether the CLI itself is installed — a repo can carry a `backlog/` folder on a machine that has never installed the tool.
...
1. Install the CLI if `command -v backlog` came back empty: `npm i -g backlog.md` (the npm package carries the dot; the command it installs is `backlog`).
2. Initialise the tracker if no `backlog/` directory or `backlog.config.yml` exists: `backlog init "<project name>"`.
3. Read the real status vocabulary with `backlog config`, and write those exact values into `docs/agents/issue-tracker.md` wherever the template names a status — the template ships with `To Do` / `In Progress` / `Done` as a placeholder shape, not as fact.
